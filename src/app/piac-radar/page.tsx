...
export default function PiacRadarPage() {
  ...
  const selected: RadarOpportunity | null =
    filtered.find((o) => o.id === (selectedId ?? filtered[0]?.id)) ?? filtered[0] ?? null;
      ...
      <section className="border-b border-[#27272a] bg-[#121214] py-10 sm:py-14">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 space-y-4 text-center flex flex-col items-center">
          ...
          <p className="text-xs sm:text-sm text-zinc-400 max-w-xl mx-auto font-normal">
            Magyar piacra rangsorolt viszonteladási lehetőségek – momentum, árrés és beszállítói út egy
            helyen.
          </p>
      ...
      <div className={`${SECTION} pt-6 space-y-6`}>
        ...
        <div className="flex flex-wrap gap-2">
          {[
            ...
            { label: "Medián eladott", value: formatHuf(medianSoldAvg), sub: "comps" },
            { label: "Átlag plafon", value: formatHuf(avgCeiling), sub: "beszerzés alatt" },
          ].map((stat) => (
            <div
              key={stat.label}
            ...
            >
              ...
              <p className="text-xs font-semibold text-white shrink-0 font-sans">{stat.value}</p>
              <p className="text-xs font-normal text-zinc-500 shrink-0 font-sans">{stat.sub}</p>
            </div>
          ))}
        </div>
        <p className="text-[11px] text-zinc-500 font-normal">
          A beszerzési plafon a marketplace díjak utáni célár, amivel még van értelme belépni.
        </p>

        {/* Main grid */}
        <div className="grid grid-cols-1 xl:grid-cols-[minmax(0,1fr)_minmax(0,1.05fr)] gap-4 items-start">
          {/* Left: opportunities + momentum */}
                          ...
                          <div className="min-w-0 flex-1 space-y-1.5">
                            <div className="flex items-start justify-between gap-2">
                              <div className="min-w-0">
                                <p className="text-sm font-black text-white truncate">{op.name}</p>
                            ...
                            <div className="flex flex-wrap gap-1.5">
                              ...
                              <span className="px-2 py-0.5 rounded-full bg-zinc-900 text-zinc-400 text-[10px] font-bold">
                                {op.soldCount} eladott
                              </span>
                              <span className="px-2 py-0.5 rounded-full bg-zinc-900 text-zinc-400 text-[10px] font-bold">
                                {op.activeCount} aktív
                              </span>
                            </div>
                            <p className="text-[10px] text-zinc-500 font-normal">
                              Medián eladott: {formatHuf(op.medianSold)}
                            </p>
                          </div>
                        </div>
                      </button>
              ...
              <>
                <div className="rounded-2xl bg-[#121214] border border-[#27272a] p-5 sm:p-6 space-y-5">
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    ...
                    <span className="px-2.5 py-1 rounded-full bg-zinc-900 border border-zinc-700 text-[10px] font-bold text-zinc-300">
                      {selected.soldCount} eladott
                    </span>
                    <span className="px-2.5 py-1 rounded-full bg-zinc-900 border border-zinc-700 text-[10px] font-bold text-zinc-300">
                      {selected.activeCount} aktív
                    </span>
                  </div>

                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-2.5">
                    {[
                      { label: "Medián eladott", value: formatHuf(selected.medianSold), accent: false },
                      { label: "Nettó resale", value: formatHuf(selected.netResale), accent: true },
                      { label: "Beszerzési plafon", value: formatHuf(selected.buyCeiling), accent: true },
                    ].map((m) => (
                      <div
                        key={m.label}
                ...
                <div className="rounded-2xl bg-[#121214] border border-[#27272a] p-5 space-y-3">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-[11px] text-zinc-500 font-normal">Magyarország</span>
                  </div>
                    ...
                    {selected.soldExamples.map((ex, i) => (
                      ...
                      >
                        ...
                        <div className="p-2 space-y-0.5">
                          ...
                          <p className="text-xs font-black text-[#ccff00]">{formatHuf(ex.price)}</p>
                        </div>
                      </div>
                    ))}
                  </div>
                    ...
                    {selected.marketplaceExamples.map((ex) => (
                      ...
                      >
                        ...
                        <div className="min-w-0 flex-1">
                          ...
                          <p className="text-[10px] text-zinc-500 font-normal">{ex.marketplace}</p>
                        </div>
                    ...
                    <div className="flex items-start gap-2.5 flex-1">
                      ...
                      <div>
                        <p className="text-[11px] font-black uppercase tracking-wider text-[#ccff00]">
                          Vendors that pass authentication
                        </p>
                        <p className="text-xs text-zinc-300 font-normal mt-1 leading-relaxed">
                          Autentikáción átmenő beszállítók a Masterclassban – ellenőrzött listák és
                          QC workflow egyben.
                        </p>
                      </div>
                    </div>
                    ...
                    {selected.vendors.map((vendor) => (
                      ...
                      >
                        <div className="min-w-0">
                          ...
                          <p className="text-[11px] text-zinc-500 font-normal">{vendor.categories}</p>
                        </div>
                        <Link
                          href={vendor.href}
